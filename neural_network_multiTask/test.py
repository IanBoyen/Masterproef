...
def run_test(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for images, class_labels_int, class_labels, reg_labels in tqdm(dl_test):  # Assuming the dataloader returns both class and reg labels
            ...
            for true_values, predicted_values, insert_type_int in zip(reg_labels.cpu().numpy(), reg_pred.cpu().numpy(), class_labels_int.cpu().numpy()):
                for true_value, predicted_value in zip(true_values, predicted_values):  # Iterate over multiple regression targets (Top, Left, Right, Bottom)
