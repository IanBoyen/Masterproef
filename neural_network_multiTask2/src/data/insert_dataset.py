...
class InsertDataset(Dataset):
    def __init__(self, data_path="../../dataset2", split=0.9, subset='train', k=5, val_fold=0, transform=None, csv_file="Wear_data.csv"):
        data_path = Path(data_path)
        df_wear = pd.read_csv(csv_file)
        wear_dict = df_wear.set_index("Insert_Name")["Wear"].to_dict()

        # Create dataframe from data
        data = []
        for img_path in data_path.glob('*/*'):
            img_name = img_path.stem 
            if img_name in wear_dict:
                data.append({
                    'image': str(img_path),
                    'label': img_path.parent.name,
                    'wear': wear_dict[img_name]
                })
            else:
                print(f"Skipping {img_path}: data not found in CSV")
    ...
    def __getitem__(self, idx):
        ...
        regression_target = torch.tensor(self.df['wear'][idx], dtype=torch.float32).unsqueeze(0)
