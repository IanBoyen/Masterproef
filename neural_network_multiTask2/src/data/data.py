...
def get_data_loaders(
    ...
    # Create transforms (Images are 224x112)
    transforms = v2.Compose([
        v2.ToImage(), #Convert Image to Pytorch tensor
        v2.ToDtype(torch.float32, scale=True), #Convert dtype
        v2.Normalize(mean=norm_mean, std=norm_std), #Normalize pixel values based on Pretrained ImageNet model
    ])
