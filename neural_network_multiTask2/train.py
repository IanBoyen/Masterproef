...
def training_epoch(
    ...
    for (train_batch_idx, batch) in enumerate(tqdm(dl_train, leave=False), start=train_batch_idx + 1):
        ...
        #print(f"Regression loss {loss_regression}")
        #print(f"Classification loss {loss_classification}")
...
def validation_epoch(
    ...
    for batch_idx, batch in enumerate(tqdm(dl_val, leave=False)):
        ...
        total_losses.append((class_loss.item() * loss_weight_class + reg_loss.item() * loss_weight_reg) / (loss_weight_class + loss_weight_reg))  # Combine both losses
        #total_losses.append(class_loss.item() + reg_loss.item())
    ...
    mean_total_losses = sum(total_losses) / len(total_losses)
    ...
    wandb.log({
        ...
        "Val/Total_Loss": mean_total_losses,
